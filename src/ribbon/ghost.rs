...
/// System-set the ghost paint system lives in. Downstream plugins
...
/// Paints a translucent accent-outlined square at the slot the
/// dragged button would land on if released right now. No-op when
/// no drag is active or when the cursor is over a ribbon the
/// dragged button's constraint forbids.
pub fn paint_drop_ghost_system(
