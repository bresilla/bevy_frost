...
use frost_core::pane::{PaneAnchor, Pane2, RailZone};
    ...
    for &(_, button_id, default_anchor, label) in PANE_DEFS {
        ...
        Pane2::new(button_id, label, anchor, accent_col)
            ...
            .show(ctx, |body_ui| match button_id {
                ...
                PANE_EDITOR     => {
                    // Build a fresh `BevyNodeViewBackend` for this
                    // pane's `editor_pane` call. The backend borrows
                    // the wgpu resources + the node-view bookkeeping
                    // resources mutably for the duration of the call;
                    // re-borrowed every loop iteration that hits this
                    // arm.
                    let mut backend = BevyNodeViewBackend::new(
                        &node_view_params.render_device,
                        &node_view_params.render_queue,
                        &mut node_view_params.images,
                        &mut node_view_params.egui_textures,
                        &mut node_view_params.pending,
                        &mut node_view_params.slots,
                    );
                    // Drive the `Time` / `Perlin` nodes from
                    // egui's input clock so they animate live.
                    let now = body_ui
                        .ctx()
                        .input(|i| i.time);
                    ...
                    editor_pane(
                        ...
                        &mut node_view_params.editor_graph.0,
                        &mut viewer,
                        &mut backend,
                    );
                }
...
fn render_containers<'a>(
    ...
    let dots_orient = if containers_stack_horizontally {
        frost_core::container::SeparatorOrient::Vertical
    } else {
        frost_core::container::SeparatorOrient::Horizontal
    };
    ...
    let mut responses: std::collections::HashMap<
        egui::Id,
        Vec<frost_core::pod::PodResponse>,
    ...
    for cid in order.into_iter() {
        ...
        let dragging_self = frost_core::pane::active_drag(body_ui.ctx())
            .and_then(|(_, s)| s.item)
            .map(|item| item == cid)
            .unwrap_or(false);
        ...
        let dot_resp =
            frost_core::pane::paint_container_dots(body_ui, dots_orient, cid, accent);
        ...
        if dot_resp.dragged() && body_open {
            let cur = frost_core::container::container_flow(
                body_ui.ctx(),
                cid,
            ...
            frost_core::container::set_container_flow(
                body_ui.ctx(),
                cid,
                cur + delta,
                pane_horizontal_strip,
...
fn walk_demo_tree(
    ...
    let resp = frost_core::widget::tree_row(
        ui,
        *p,
        depth,
