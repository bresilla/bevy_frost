...
use crate::style::{
    contrast_text_for, font, glass_alpha_group, glass_fill, on_section, subsection_fill,
    theme, widget_border,
};
...
pub fn chip_colored(
    ...
    let text_col = if fill.a() >= 220 { contrast_text_for(fill) } else { on_section() };
    let galley = {
    ...
    if ui.is_rect_visible(rect) {
        ui.painter().rect(
            ...
            fill,
            egui::Stroke::new(theme().border_width, widget_border(accent)),
