...
use egui::{vec2, Sense, Vec2};

use crate::style::{on_section, UNIT};
...
/// Body font size used for the label.
const LABEL_FONT: f32 = 11.0;
/// Inner horizontal padding so the label doesn't kiss the section
/// edge.
const LABEL_PAD_X: f32 = 8.0;
/// Gap between label cell and the first chip.
const LABEL_TO_CHIPS_GAP: f32 = 6.0;
/// Gap between adjacent chips, in px.
const CHIP_GAP: Vec2 = vec2(3.0, 0.0);
...
pub fn badge_row_colored(
    ...
    let fills: Vec<Option<egui::Color32>> =
        badges.iter().map(|(_, f)| *f).collect();
...
fn badge_row_with(
    ...
    let label_w = BADGE_LABEL_COL_W.min(total_w * 0.5);
    ...
    let (rect, resp) =
        ui.allocate_exact_size(vec2(total_w, BADGE_ROW_H), Sense::hover());
    if !ui.is_rect_visible(rect) {
        return resp;
    }

    // ── Label cell ──
    let label_rect = egui::Rect::from_min_size(rect.min, vec2(label_w, BADGE_ROW_H));
    let painter = ui.painter_at(label_rect);
    let label_font = egui::FontId::proportional(LABEL_FONT);
    let mut job = egui::text::LayoutJob::single_section(
        label.to_string(),
        egui::TextFormat::simple(label_font, on_section()),
    );
    job.wrap.max_rows = 1;
    job.wrap.max_width = (label_w - LABEL_PAD_X).max(0.0);
    job.wrap.break_anywhere = true;
    let galley = painter.layout_job(job);
    painter.galley(
        egui::pos2(
            label_rect.min.x + LABEL_PAD_X,
    ...
    let badges_x = rect.min.x + label_w + LABEL_TO_CHIPS_GAP;
    let badges_rect = egui::Rect::from_min_max(
        egui::pos2(badges_x, rect.min.y),
        rect.max,
    );
    ...
    child.spacing_mut().item_spacing = CHIP_GAP;
