...
pub struct NodeViewState {
    ...
    /// `true` once we've called `install_fonts` on `sub_ctx`.
    /// Without this the sub context has no Fluent UI / iconflow
    /// font families bound, and any caller painting an icon glyph
    /// (`corekit::icons::icon_text`) inside the body callback
    /// panics with "FontFamily::Name(...) is not bound to any
    /// fonts" the first frame.
...
pub fn show_with_anchor<R>(
    ...
    // First-frame: install the same font set the parent has on
    // the sub context so `corekit::icons::icon_text` etc. work
    // when painted INSIDE the secondary context. We can't simply
    // mirror `ctx.fonts(...)` (it returns the resolved cache, not
    // the source `FontDefinitions`), so we re-run the public
    // `install_fonts` builder with the global weight selectors.
    ...
    // Mirror the parent's frost theme onto the sub-context every
    // frame so EVERY native egui widget the body callback paints
    // (`DragValue`, `ComboBox`, `Slider`, `Checkbox`,
    // `color_edit_button_srgba`, …) inherits the frost palette,
    // input fills, corner radii, button paddings and spacing
    // exactly like the parent UI.
    //
    // `apply_theme` itself early-returns when the theme hasn't
    // changed since its last call — that cache is global, not
    // per-context, so once the parent ctx has been styled the
    // dedup gate fires and a follow-up call here would be a
    // no-op leaving the sub-context unstyled. `apply_theme_to`
    // is the bypass: it always pushes the current visuals onto
    // the supplied ctx without consulting the cache.
